...
CustomHidChangeHandler(void)
{
    ...
    //
    signed char DeviceReport[1]; //The array isn't needed but keeps consistent with reports with >1 byte
    DeviceReport[0]=0; //Don't normally send any data



    //
    //Read Left button and decrease volume when pushed
    //
    ulButton = ROM_GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_4);
    if(ulButton == 0)
    {
    	DeviceReport[0]=2; //Volume Up (00000010)
    }

    //
    //Read Right button and increase volume
    //
    ulButton2 = ROM_GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0);
    if(ulButton2 == 0)
    {
    	DeviceReport[0]=1; //Volume Down (00000001)
    }
